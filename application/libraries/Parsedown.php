...
//        var_dump($Elements, '------------------------');
	...
	protected function linesElements(array $lines) {
		...
		foreach ($lines as $line) {
			...
			$indent = strspn($line, ' '); // 缩进
			$text = $indent > 0 ? substr($line, $indent) : $line; // 拿到去除缩进后的文本
			# ~
			$Line = array('body' => $line, 'indent' => $indent, 'text' => $text);
			# 像 代码块，列表，表格，就是跨行，需要继续解析下一行
	...
	protected $InlineTypes = array(
		...
		'_' => array('Emphasis'), // 加粗
		'`' => array('Code'),
		'~' => array('Strikethrough'), // 删除线
		'\\' => array('EscapeSequence'), // 转义
		'(' => array('Pinyin'),
	);
	...
	protected function lineElements($text, $nonNestables = array()) {
		...
		// 在字符串中查找一组字符的任何一个字符（查找当前前是否有 markdown 语法）
	...
	/**
	 * 少数民族之ｃｈāｎｇｚú 名 我国少数民族之一，分布在jìngyǎng云南
	 * @param $Excerpt
	 * @return array|void
	 */
	protected function inlinePinyin($Excerpt) {
		$reg = '/^[(]\s*([\x{4e00}-\x{9fa5}]+)\s*\|((?:[^()]+|[(][^)]+[)])+)\s*[)]/u';
	...
	protected function element(array $Element) {
		...
		if (isset($Element['text'])) { // true : 说明是纯文本元素了
			$text = $Element['text'];
		} elseif (isset($Element['rawHtml'])) { // very strongly consider an alternative if you're writing an extension
